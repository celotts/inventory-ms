...
public record ApiResponse<T>(
        ...
        T data,
        Long total,
        String path,        // opcional (se llena en errores)
        String method,      // opcional (se llena en errores)
        String errorCode,   // opcional (códigos internos: ERR_* )
        List<Violation> errors // opcional (errores de validación)
) {
    // ----------------- ÉXITO -----------------
    ...
    public static <T> ApiResponse<T> badRequest(String message, String path, String method, List<Violation> errors) {
        return error(400, message, path, method, "ERR_BAD_REQUEST", errors);
    }
