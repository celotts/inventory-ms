...
@FeignClient(name = "supplier-service", url = "${supplier.service.url:http://localhost:9091}")
public interface SupplierClient {

    @GetMapping("/supplier-service/api/v1/suppliers/{id}")
    SupplierDto getSupplier(@PathVariable("id") UUID id);

    @PostMapping("/supplier-service/api/v1/suppliers")
